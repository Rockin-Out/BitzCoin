...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitzCoinAmount"))
        settings.setValue("nAnonymizeBitzCoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeBitzCoinAmount = settings.value("nAnonymizeBitzCoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeBitzCoinAmount"))
        SoftSetArg("-anonymizeBitzCoinamount", settings.value("nAnonymizeBitzCoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeBitzCoinAmount:
            nAnonymizeBitzCoinAmount = value.toInt();
            settings.setValue("nAnonymizeBitzCoinAmount", nAnonymizeBitzCoinAmount);
            emit anonymizeBitzCoinAmountChanged(nAnonymizeBitzCoinAmount);
            break;
        default:
            break;
        }
