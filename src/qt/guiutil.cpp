...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("BitzCoin"))
        return false;
...
{
    // Convert BitzCoin:// to BitzCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BitzCoin://"))
    {
        uri.replace(0, 12, "BitzCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitzCoin.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IBitzCoinLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IBitzCoinLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "BitzCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitzCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BitzCoin-qt [" + tr("command-line options") + "]                     " + "\n";
